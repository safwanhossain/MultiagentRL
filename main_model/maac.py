...
class MAAC(BaseModel):
    ...
    def __init__(self, flags, envs, critic_arch, policy_arch, batch_size, seq_len, discount, lam,
        ...
        super(MAAC, self).__init__(envs, batch_size, seq_len, discount, lam,
        ...
        self.critic = MAAC_Critic(observation_size=self.obs_size, action_size=self.action_size,
                                  num_agents=self.n_agents, attention_heads=4, embedding_dim=128,
                                  device=self.device)
    ...
    def update_actor(self):
        ...
        all_q, q_vals = self.critic.forward((self.observations, self.actions), ret_all_actions=True)
        all_q.detach()
        q_vals.detach()

        # print("q_vals", q_vals)
        diff_action_in = self.actions
    ...
    def td_one(self):
        ...
        G = torch.zeros((self.n_agents, self.batch_size, self.seq_len, 2)).to(self.device)

        # initialize the first column with estimates from the target Q network
        predictions = self.target_critic.forward((self.observations, self.actions)).squeeze()

        # use detach to stop tracking gradients
        G[:, :, :, 0] = predictions.detach()
        ...
        for t in range(self.seq_len - 2, -1, -1):
            ...
            pred = self.critic.forward((self.observations[:, :, t:t + 1, :], self.actions[:, :, t:t + 1, :])).squeeze()
    ...
    def td_lambda(self):
        ...
        for t in range(self.seq_len - 2, -1, -1):
            ...
            pred = self.critic((self.observations[:, :, t:t + 1, :], self.actions[:, :, t:t + 1, :])).squeeze()
            # print('pred', pred[0])
