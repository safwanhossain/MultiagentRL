...
class SCIIEnv:
    ...
    def allot_observations(self, timestep):
        ...
        agent_observations = []
        global_observations = np.zeros([SCIIEnv.NUM_TOTAL, 5])
        # Get xy of all units
        xy = np.zeros([len(obs.raw_units), 2])

        global_idx_self = 0
        global_idx_other = SCIIEnv.NUM_TOTAL // 2
        unit_ids = []
        ...
        for agent in self.agents:
            ...
            agent_obs = np.zeros([SCIIEnv.NUM_ALLIES + SCIIEnv.NUM_OTHERS, 6])
            ...
            # setup indexing
            obs_self_idx = 0
            obs_enemy_idx = SCIIEnv.NUM_ALLIES
...
if __name__ == "__main__":
    sc2 = SCIIEnv("FindAndDefeatZerglings", TestAgent, 1000, 1, visualize=True)
    sc2.start()
